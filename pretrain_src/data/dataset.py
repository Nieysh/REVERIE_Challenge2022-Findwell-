...
class ReverieTextPathData(object):
    def __init__(
        ...
        max_txt_len=100, in_memory=True, act_visited_node=False,args = None, obj_feats = None
    ):
        self.args = args
        ...
        self.all_point_angle_fts = [get_angle_fts(x[:, 0], x[:, 1], self.angle_feat_size) for x in self.all_point_rel_angles]  #self.angle_feature
    ...
    def get_traj_pano_fts(self, scan, path):
        ...
        for vp in path:
            ...
            view_angles.extend(
                [self.all_point_rel_angles[12][idx] for idx in range(36) if idx not in used_viewidxs])
            # combine cand views and noncand views
            view_img_fts = np.stack(view_img_fts, 0)  # (n_views, dim_ft)
            view_angles = np.stack(view_angles, 0)
            view_ang_fts = get_angle_fts(view_angles[:, 0], view_angles[:, 1],
                                         self.angle_feat_size)  # directional angel
            view_box_fts = np.array([[1, 1, 1]] * len(view_img_fts)).astype(np.float32)

            if self.args.new_data:
                # print("begin loading new obj feature in dataset.py")
                # obj feature
                base_view_id = 12  # base_heading = 0; base_elevation = 0
                directional_feature = self.all_point_angle_fts[base_view_id]  # 核对过
                obj_angles = []
                obj_ang_fts = []
                obj_box_fts = []
                obj_img_fts = []
                ...
                for vis_pos, objects in obj_feats[scan][vp].items():
                    for objId, obj in objects.items():
                        if int(vis_pos) < 25:
                            # print(self.all_point_rel_angles[12][int(vis_pos)])
                            obj_angles.append(self.all_point_rel_angles[12][int(vis_pos)]) #TODO
                ...
                obj_angles = np.array(obj_angles, dtype=np.float32)
                not_int(obj_img_fts,"obj_img_fts")
                not_int(obj_ang_fts,"obj_ang_fts")
                not_int(obj_box_fts,"obj_box_fts")
                not_int(obj_angles,"obj_angles")
            ...
            # last_vp_objids = obj_attrs.get('obj_ids', [])  #TODO
            # last_vp_angles = np.concatenate([view_angles, obj_angles], 0)  #TODO
            if self.args.new_data:
                last_vp_objids = obj_ids
...
def not_int(x,name):
    print("{} size: {}".format(name,x.shape))
