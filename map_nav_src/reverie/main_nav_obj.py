...
import sys
sys.path.append("/data1/nys_new/VLN-DUET/map_nav_src")
sys.path.append("/data1/nys_new/VLN-DUET")
...
from env import loadObjProposals
...
def build_dataset(args, rank=0):
    ...
    obj_feats = None
    if args.old_obj_setting:
        print("old_obj_setting : True") #用于验证旧obj feature与新obj feature对结果的影响
        obj2vps = {}
    else:
        if args.new_data:
            print("newdata:读取新obj特征文件")
            with open('../datasets/REVERIE/features/reverie_obj_feats_v2.pkl',
                      'rb') as f_obj:
                obj_feats = pkl.load(f_obj)
            obj2vps = {}
            print("newdata:读取新obj特征文件完毕")
        else:
    ...
    if args.new_data:
        tokenizer = AutoTokenizer.from_pretrained(args.cfg_name)
    else:
        tokenizer = args.tokenizer
    if args.aug is not None:
        aug_instr_data = construct_instrs(
            args.anno_dir, args.dataset, [args.aug], 
            tokenizer=tokenizer, max_instr_len=args.max_instr_len,args =args
        )
        aug_env = dataset_class(
            feat_db, obj_db, obj_feats, aug_instr_data, args.connectivity_dir, obj2vps,
    ...
    if args.aug_only:
        ...
        train_env = dataset_class(
            feat_db, obj_db, obj_feats, train_instr_data, args.connectivity_dir, obj2vps,
            batch_size=args.batch_size, max_objects=args.max_objects,
    ...
    for split in val_env_names:
        ...
        val_env = dataset_class(
            feat_db, obj_db, obj_feats, val_instr_data, args.connectivity_dir, obj2vps, batch_size=args.batch_size,
            angle_feat_size=args.angle_feat_size, seed=args.seed+rank,
