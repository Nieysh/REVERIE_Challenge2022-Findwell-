...
def load_instr_datasets(anno_dir, dataset, splits, tokenizer, args):
    data = []
    for split in splits:
        if "/" not in split:    # the official splits
            if args.new_data:
                filepath = os.path.join("../datasets/REVERIE/annotations", 'REVERIE_%s.json' % split)
            else:
                if tokenizer == 'bert':
...
def construct_instrs(anno_dir, dataset, splits, tokenizer, max_instr_len=512, args = None):
    data = []
    for i, item in enumerate(load_instr_datasets(anno_dir, dataset, splits, tokenizer, args)):
...
def load_obj2vps_new(bbox_file):
    obj2vps = {}
    bbox_data = json.load(open(bbox_file))
    for scanvp, value in bbox_data.items():
        scan, vp = scanvp.split('_')
